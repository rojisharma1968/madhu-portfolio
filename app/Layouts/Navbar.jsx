  ...
  return (
    <motion.nav className="fixed size-fit md:w-[36rem] top-2 md:bottom-3 md:top-[unset] z-50 right-3 md:left-1/2 md:-translate-x-1/2">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="menu border-2 w-fit px-3 absolute right-2 top-14  md:hidden border-dashed border-[var(--color-purple)] drop-shadow-2xl flex flex-col justify-center py-3 rounded-xl gap-4 bg-[var(--coloroff-white)] z-40"
          >
            <motion.div
      ...
      <motion.div
        ...
        className="hidden md:flex border-2 w-fit px-3 border-dashed border-[var(--color-purple)] drop-shadow-2xl flex-row justify-center py-2 rounded-xl gap-4 bg-[var(--coloroff-white)] z-40 lg:mx-auto"
      >
        {links.map((link) => (
          <Link
            key={link.text}
